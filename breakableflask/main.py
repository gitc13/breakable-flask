...
def log_interaction(input_data, output_data, route_name):
    """Log input and output to route-specific log files in format: datestamp:input, output"""
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    log_entry = f"{timestamp}:{input_data}, {output_data}\n"
    log_filename = f"server-trace-{route_name}.log"
    
    try:
        with open(log_filename, 'a', encoding='utf-8') as log_file:
            log_file.write(log_entry)
    except Exception as e:
        print(f"Logging error for {route_name}: {e}")
...
def cookie():
    ...
    if request.method == 'POST':
        ...
        value = cookieValue
        input_data = f"POST /cookie value={cookieValue}"
    elif 'value' in request.cookies:
        cookieValue = pickle.loads(b64decode(request.cookies['value']))
        input_data = f"GET /cookie cookie_value={request.cookies['value'][:50]}..."
    else:
        input_data = "GET /cookie (no cookie)"
    ...
    # Log the interaction
    output_data = f"Cookie value displayed: {str(cookieValue)[:100]}..."
    log_interaction(input_data, output_data, "cookie")
...
def lookup():
    ...
    if request.method == 'POST':
        ...
        input_data = "GET /lookup (form display)"
        
    # Log the interaction
    output_data = f"DNS lookup result: {result[:200]}..." if result else "Form displayed"
    log_interaction(input_data, output_data, "lookup")
...
def evaluate():
    ...
    if request.method == 'POST':
        ...
        input_data = f"POST /evaluate expression={expression}"
        try:
            result = str(eval(expression)) if expression else ""
        except Exception as e:
            result = f"Error: {str(e)}"
    else:
        input_data = "GET /evaluate (form display)"
        
    # Log the interaction
    output_data = f"Evaluation result: {result[:200]}..." if result else "Form displayed"
    log_interaction(input_data, output_data, "evaluate")
...
def xml():
    ...
    if request.method == 'POST':
        ...
        input_data = f"POST /xml xml={xml_input[:100]}..."
        parser = etree.XMLParser(no_network=False, dtd_validation=False, load_dtd=True, huge_tree=True)
        try:
            doc = etree.fromstring(xml_input.encode(), parser)
            parsed_xml = etree.tostring(doc).decode('utf8')
        except Exception as e:
            parsed_xml = f"XML parsing error: {str(e)}"
    else:
        input_data = "GET /xml (form display)"
        
    # Log the interaction
    output_data = f"XML parsed result: {parsed_xml[:200]}..." if parsed_xml else "Form displayed"
    log_interaction(input_data, output_data, "xml")
...
def config():
    ...
    input_data = f"GET /config key={key}" if key else "GET /config (no key)"
    ...
    if key:
        try:
            ...
            output_data = f"Config access error: {str(e)}"
            log_interaction(input_data, output_data, "config")
            return str(e)
    ...
    # Log the interaction
    output_data = f"Config accessed - key: {decrypted_key}, value: {str(config_out)[:100]}..." if decrypted_key else "Config list displayed"
    log_interaction(input_data, output_data, "config")
...
def sayhi():
   ...
   if request.method == 'POST':
      ...
      name = '<br>Hello %s!<br><br>' %(user_name)
      input_data = f"POST /sayhi name={user_name}"
   else:
      input_data = "GET /sayhi (form display)"
   ...
   # Log the interaction
   output_data = f"Greeting displayed: {name[:100]}..." if name else "Form displayed"
   log_interaction(input_data, output_data, "sayhi")
...
def listservices():
    ...
    input_data = f"GET /listservices category={category}" if category else "GET /listservices (no filter)"
    ...
    try:
        query = query_build('SELECT * from public_stuff{}'.format(where))
        cursor.execute(query)
        results = cursor.fetchall()
        output_data = f"SQL query executed: {query}, returned {len(results)} rows"
    except Exception as e:
        output_data = f"SQL error: {str(e)}"
        log_interaction(input_data, output_data, "listservices")
        return str(e)
